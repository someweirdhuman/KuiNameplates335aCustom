...
local speed = 0.05
local function getSpeedFactor(newX, newY, oldX, oldY)
    -- Calculate 2D distance
    local distance = math.sqrt((newX - oldX)^2 + (newY - oldY)^2)

    local maxDistance = 50
    local minSpeed = 0.04
    local maxSpeed = 0.1

    -- Inverse lerp to smoothly map the distance
    local t = math.min(distance / maxDistance, 1)

    -- Reverse the value (so small distance = high speed)
    return maxSpeed - t * (maxSpeed - minSpeed)
end

ticker:SetScript("OnUpdate", function(self, elapsed)


		for k, frame in pairs(activeFrames) do
			if frame.kui.prevX == nil then
				frame.kui.prevX = 0
				frame.kui.prevY = 0
			end
			if frame.kui.newX and frame.kui.prevX then
				local newSpeed = getSpeedFactor(frame.kui.newX, frame.kui.prevX, frame.kui.newY, frame.kui.prevY)
				local newX = floor(frame.kui.prevX + ((frame.kui.newX - frame.kui.prevX) * newSpeed))
				local newY = floor(frame.kui.prevY + ((frame.kui.newY - frame.kui.prevY) * newSpeed / 3))
				print(newSpeed)
				frame.kui.prevX = newX
				frame.kui.prevY = newY
				frame.kui:SetPoint("CENTER", WorldFrame, "BOTTOMLEFT", newX, newY)
			end
			--printtable(frame)
		end
...
local function OnFrameShow(self)
	local f = self.kui
	local frameID = tostring(self)
    activeFrames[frameID] = self
